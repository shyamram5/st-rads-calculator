...
/*
  ...
  Per manuscript AND rule engine (scoreCystlike):
  ...
  NOTE: The rule engine checks cystHematoma on the no-flow path (line 140).
  The wizard only asks cystHematoma when flow="yes", and only asks cystSeptationNodules
  when flow="no". However, the manuscript figure shows hematoma on both paths, and the 
  rule engine supports it. The flowchart follows the manuscript.
*/
...
export default function Figure2BChart({ caseData, finalScore }) {
  ...
  // No-flow path: rule engine checks hematoma first, then septations
  const hNoFlowHemYes = hFlowNo && hem === "yes";
  const hNoFlowHemNo = hFlowNo && (hem === "no" || !hem);
      ...
      <TreeFork parentHighlighted>
        ...
        <div className="flex flex-col items-center">
          ...
          <TreeFork parentHighlighted={hDeep}>
            ...
            {/* No flow voids → hematoma fork → then septation fork */}
            <div className="flex flex-col items-center">
