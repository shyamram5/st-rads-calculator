...
/*
  ...
  Matches scoreLipomatous() in stradsRuleEngine.js exactly.

  "Lipomatous" soft tissue lesion
  ├─ Predominantly lipomatous (>90%)
  │   ├─ Thin septations (<2 mm) OR absence of nodules → ST-RADS 2
  │   └─ Septations AND presence of nodules
  │       ├─ Thin septations <2 mm OR enhancement <10%
  │       │   ├─ Many prominent vessels → ST-RADS 3 (Angiolipoma)
  │       │   └─ Few prominent vessels → ST-RADS 4 (ALT/WDL)
  │       └─ Thick septations ≥2 mm OR enhancement >10% → ST-RADS 4
  └─ Not predominantly lipomatous (≤90%)
      ├─ No enhancing nodule(s) OR proportionately larger lipomatous component → ST-RADS 4
      └─ Enhancing nodule(s) OR proportionately smaller lipomatous component → ST-RADS 5
*/
      ...
      <TreeFork parentHighlighted>
        ...
        <div className="flex flex-col items-center">
          ...
          <TreeFork parentHighlighted={hPred}>
            {/* Thin/absence → RADS 2 */}
            <div className="flex flex-col items-center">
              <N label="Thin septations (<2 mm) OR absence of nodules, like subcutaneous fat on all sequences" isHighlighted={hThinAbsence} className="max-w-[130px]" />
              <Stem h={hThinAbsence} />
              <N type="score" score={2} isHighlighted={hThinAbsence} isActive={hThinAbsence && finalScore === 2} />
            </div>

            {/* Septations AND nodules present */}
            <div className="flex flex-col items-center">
              <N label="Septations AND presence of nodules, like subcutaneous fat on all sequences" isHighlighted={hSeptPresence} className="max-w-[130px]" />
